...
function(y, var1, contrastmodel = NULL) {
  ...
  for ( j in 1: length(levels(var1))){
    ...
    L_name = paste0(L, collapse='|')
    
    var2 <- as.character(var1)
    for ( i in 1:length(var1)){
      if (var1[i] %in% L)  {(var2[i] = L_name) }
      else {(var2[i] = R)}
    }
    var2 <- factor(var2)
    ...
    x<-c(x2,pvalue,x2_2_1,p_2_1,Mean_1, Mean_2, fc_2_1, score_2_1, abs_score_2_1)
    names = c("x2_2_1","pvalue_2_1","Chi_2_1","P_2_1", "Mean_1", "Mean_2", "fc_2_1", "score_2_1", "abs_score_2_1")
    names = sub("1",levels(var2)[1],names)
    names = sub("2", levels(var2)[2], names)
    # names(x)[1] = str_replace(names(x)[1], pattern="$$$", replacement="2")
    names(x) = names
